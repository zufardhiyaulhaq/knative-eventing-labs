...
func receive(ctx context.Context, event cloudevents.Event) (*cloudevents.Event, cloudevents.Result) {
	...
	imageResize := imaging.Resize(image, 512, 512, imaging.NearestNeighbor)

	buffer := new(bytes.Buffer)
	err = imaging.Encode(buffer, imageResize, 1)
	if err != nil {
	...
	name := strings.TrimSuffix(eventData.Name, filepath.Ext(eventData.Name)) + "-resize"
